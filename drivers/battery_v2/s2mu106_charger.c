...
static void s2mu106_test_read(struct i2c_client *i2c)
{
	...
	s2mu106_read_reg(i2c, 0xF3, &data);
	sprintf(str+strlen(str), "0xF3:0x%02x, ", data);
		...
		struct s2mu106_charger_data *charger, int float_voltage)
{
	...
	if (factory_mode) {
		pr_info("%s: Factory Mode Skip Regulation Voltage Setting\n", __func__);
		return;
	}
		...
		struct s2mu106_charger_data *charger, int input_current)
{
	...
	pr_info("[DEBUG]%s: input current %d, 0x%x\n", __func__, input_current, data);
...
static int s2mu106_get_input_current_limit(struct s2mu106_charger_data *charger)
{
	...
	pr_info("[DEBUG]%s: current: 0x%x, %d\n", __func__, data, ((data * 25) + 50));
	return ((data * 25) + 50);
}
		...
		struct s2mu106_charger_data *charger, int charging_current)
{
	...
	pr_info("[DEBUG]%s: output current %d, 0x%02x\n", __func__, charging_current, data);
		...
		struct s2mu106_charger_data *charger)
{
	...
	pr_info("[DEBUG]%s: current: 0x%02x, %d\n", __func__, data, (data + 1) * 50);
	return (data + 1) * 50;
}
...
static bool s2mu106_chg_init(struct s2mu106_charger_data *charger)
{
	...
	if (charger->pdata->chg_ocp_disable) {
		/* BAT_OCP Qbat on */
		/* do not power off when hw bat ocp occured */
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval) {
...
#endif
			/* SYS Output, default:4.0V (3.7V~4.4V Option) */
			s2mu106_set_regulation_vsys(charger, charger->pdata->vsys_factory);
			...
			s2mu106_update_reg(charger->i2c, S2MU106_CHG_CTRL8,
								0 << EN_JIG_REG_AP_SHIFT, EN_JIG_REG_AP_MASK);
		...
		struct s2mu106_charger_platform_data *pdata)
{
	...
	if (!np) {
		...
		ret = of_property_read_u32(np, "charger,vsys_factory",
				&pdata->vsys_factory);
		if (ret) {
			pr_info("%s: charger,vsys_factory is Empty, set to default 4000mV\n",
				__func__);
			pdata->vsys_factory = 4000;
		}
		pr_info("%s: charger,vsys_factory is %d\n",
				__func__, pdata->vsys_factory);
